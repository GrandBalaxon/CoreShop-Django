...
- регистрация с подтверждением email (верификация по токену) и авторизация через стандартный LoginView с кастомной формой
- личный кабинет с вкладками «Обзор», «Мои товары», «Мои блоги»
- редактирование профиля (аватар, телефон, страна, имя и email)
- ограничение доступа: неаутентифицированные пользователи перенаправляются на страницу входа (`login_required` на защищённых CBV)
- защита от редактирования/удаления чужих товаров и статей: примеси `OwnerRequiredMixin` и `AuthorRequiredMixin`, перебрасывают не‑владельцев на кастомную `403.html` страницу
...
Загрузите категории, продукты, пользователей, группы и контактные данные из фикстуры `test_data_fixture.json` в корне проекта:
```bash
...
* Регистрация: http://127.0.0.1:8000/users/register/
   ...
   После отправки формы на указанный email придёт письмо со ссылкой для активации аккаунта.

* Вход: http://127.0.0.1:8000/users/login/
* Профиль: http://127.0.0.1:8000/users/profile/

   Внутри доступны вкладки «Обзор», «Мои товары», «Мои блоги».

* Редактирование профиля: http://127.0.0.1:8000/users/profile/edit/
...
│   │   ├── my_blogs.html
│   │   ├── my_products.html
│   │   ├── profile_base.html  # базовый шаблон профиля (аватар, меню вкладок)
│   │   ├── profile_edit.html
│   │   ├── profile_overview.html
│   │   └── register.html
│   │       
│   ├── models.py            # CustomUser (AbstractUser с полями avatar, phone_number, country)
│   ├── views.py             # CBV для профиля, регистрации, обновления
│   ├── forms.py             
│   ├── urls.py
│   ├── admin.py
│   └── apps.py
